...
async function engineInit(
  ...
  function engineUpdate(frameTimeMS = 0) {
    ...
    if (debug || showWatermark)
      averageFPS = lerp(averageFPS, 1e3 / (frameTimeDeltaMS || 1), 0.05);
    const debugSpeedUp = debug && keyIsDown("Equal"); // +
    const debugSpeedDown = debug && keyIsDown("Minus"); // -
    if (debug)
      // +/- to speed/slow time
      frameTimeDeltaMS *= debugSpeedUp ? 10 : debugSpeedDown ? 0.1 : 1;
    timeReal += frameTimeDeltaMS / 1e3;
    frameTimeBufferMS += paused ? 0 : frameTimeDeltaMS;
    if (!debugSpeedUp) frameTimeBufferMS = min(frameTimeBufferMS, 50); // clamp min framerate
    ...
    if (paused) {
      ...
      for (; frameTimeBufferMS >= 0; frameTimeBufferMS -= 1e3 / frameRate) {
        ...
        gameUpdatePost();
        inputUpdatePost();
        if (debugVideoCaptureIsActive()) renderFrame();
      }
...
function debugRect(
  ...
  if (typeof size === "number") size = vec2(size); // allow passing in floats
  if (isColor(color)) color = color.toString();
  pos = pos.copy();
  size = size.copy();
 ...
 *  @param {boolean} [screenSpace]
 *  @memberof Debug */
function debugLine(posA, posB, color, width = 0.1, time, screenSpace = false) {
...
function debugShowErrors() {
  const showError = (message) => {
    ...
    document.body.innerHTML =
      `<pre style=color:#f00;font-size:28px;white-space:pre-wrap>` + message;
  };
...
function debugRender() {
  if (debugVideoCaptureIsActive()) return; // don't show debug info when capturing video
  ...
  if (debugOverlay) {
    ...
    if (tileCollisionTest(mousePos)) {
      ...
      drawRect(mousePos.floor().add(vec2(0.5)), vec2(1), rgb(1, 1, 0, 0.5));
    }
...

  {
    ...
    if (debugOverlay) {
      ...
      overlayContext.fillText(
        "FPS: " + averageFPS.toFixed(1) + (glEnable ? " WebGL" : " Canvas2D"),
        x,
        (y += h)
      );
      ...
      for (const i in inputData[0]) {
        if (!keyIsDown(i, 0)) continue;
        if (parseInt(i) < 3) mousePressed += i + " ";
        else if (keyIsDown(i, 0)) keysPressed += i + " ";
      }
      mousePressed &&
        overlayContext.fillText("Mouse: " + mousePressed, x, (y += h));
      keysPressed &&
        overlayContext.fillText("Keys: " + keysPressed, x, (y += h));
...
let debugVideoCapture,
  debugVideoCaptureTrack,
  debugVideoCaptureIcon,
  debugVideoCaptureTimer;

/** Check if video capture is active
 *  @memberof Debug */
function debugVideoCaptureIsActive() {
  return !!debugVideoCapture;
}

/** Start capturing video
 *  @memberof Debug */
function debugVideoCaptureStart() {
  if (debugVideoCaptureIsActive()) return; // already recording
  ...
  debugVideoCapture = new MediaRecorder(stream, {
...
function debugVideoCaptureStop() {
  if (!debugVideoCaptureIsActive()) return; // not recording

  // stop recording
  LOG(
    `Video capture ended. ${debugVideoCaptureTimer
      .get()
      .toFixed(2)} seconds recorded.`
  );
...
function debugVideoCaptureUpdate() {
  ...
  debugVideoCaptureIcon.textContent =
    "● REC " + formatTime(debugVideoCaptureTimer);
}
    ...
    props.forEach((prop) => {
      Object.defineProperty(obj, prop, {
        ...
        set: (value) => {
          ASSERT(
            false,
            `Cannot modify engine constant. Attempted to set constant (${obj}) property '${prop}' to '${value}'.`
          );
        },
        enumerable: true,
      });
...
function lerpWrap(valueA, valueB, percent, wrapSize = 1) {
  if (
    ...
    console.warn(
      "lerpWrap() parameter order changed! use lerpWrap(start, end, p)"
    );
 ...
 *  @return {number}
 *  @memberof Utilities */
function lerpAngle(angleA, angleB, percent) {
  return lerpWrap(angleA, angleB, percent, 2 * PI);
}
 ...
 * @memberof Utilities */
function isString(s) {
  return s !== undefined && s !== null && typeof s.toString() === "string";
}
...
function lineTest(posStart, posEnd, testFunction, normal) {
  ...
  while (t < totalLength) {
    if (testFunction(pos)) {
      ...
      const e = 1e-9;
      if (wasX) {
        if (stepX < 0) hitPos.x -= e;
      }
      if (stepY < 0) hitPos.y -= e;

      // set normal
      if (normal) wasX ? normal.set(-stepX, 0) : normal.set(0, -stepY);
      return hitPos;
    }
...
function randInt(valueA, valueB = 0) {
  return floor(rand(valueA, valueB));
}
...
function randInCircle(radius = 1, minRadius = 0) {
  return radius > 0
    ? randVec2(radius * rand(minRadius / radius, 1) ** 0.5)
    : new Vector2();
}
...
class RandomGenerator {
   ...
   *  @return {number} */
  int(valueA, valueB = 0) {
    return floor(this.float(valueA, valueB));
  }
  ...
  floatSign(valueA = 1, valueB = 0) {
    return this.float(valueA, valueB) * this.sign();
  }

  /** Returns a random angle between -PI and PI
   *  @return {number} */
  angle() {
    return this.float(-PI, PI);
  }
   ...
   * @return {Color} */
  mutateColor(color, amount = 0.05, alphaAmount = 0) {
...
function isVector2(v) {
  return v instanceof Vector2 && v.isValid();
}

// vector2 asserts
function ASSERT_VECTOR2_VALID(v) {
  ASSERT(isVector2(v), "Vector2 is invalid.", v);
}
function ASSERT_NUMBER_VALID(n) {
  ASSERT(isNumber(n), "Number is invalid.", n);
}
 ...
 * @memberof Utilities */
function isColor(c) {
  return c instanceof Color && c.isValid();
}

// color asserts
function ASSERT_COLOR_VALID(c) {
  ASSERT(isColor(c), "Color is invalid.", c);
}
...
class Color {
  ...
  HSLA() {
    ...
    if (maxC !== minC) {
      ...
      if (r === maxC) h = (g - b) / d + (g < b ? 6 : 0);
      else if (g === maxC) h = (b - r) / d + 2;
      else if (b === maxC) h = (r - g) / d + 4;
    }
    return [h / 6, s, l, a];
  }
  ...
  toString(useAlpha = true) {
    if (debug && !this.isValid()) return `#000`;
    const toHex = (c) =>
 ...
 *  @type {Color}
 *  @memberof Utilities */
const GRAY = debugProtectConstant(rgb(0.5, 0.5, 0.5));
 ...
 *  @type {Color}
 *  @memberof Utilities */
const ORANGE = debugProtectConstant(rgb(1, 0.5, 0));
 ...
 *  @type {Color}
 *  @memberof Utilities */
const PURPLE = debugProtectConstant(rgb(0.5, 0, 1));
...
class Timer {
  ...
  constructor(timeLeft, useRealTime = false) {
    ASSERT(
      timeLeft === undefined || isNumber(timeLeft),
      "Constructed Timer is invalid.",
      timeLeft
    );
  ...
  setUseRealTime(useRealTime = true) {
    ASSERT(
      !this.isSet(),
      "Cannot change global time setting while timer is set."
    );
    this.useRealTime = useRealTime;
  }
   ...
   * @return {number} */
  get() {
    return this.isSet() ? this.getGlobalTime() - this.time : 0;
  }
  ...
  getPercent() {
    return this.isSet()
      ? 1 - percent(this.time - this.getGlobalTime(), 0, this.setTime)
      : 0;
  }
   ...
   * @return {number} */
  getSetTime() {
    return this.isSet() ? this.setTime : 0;
  }

  /** Get the current global time this timer is based on
   * @return {number} */
  getGlobalTime() {
    return this.useRealTime ? timeReal : time;
  }
  ...
  toString() {
    return this.isSet()
      ? abs(this.get()) + " seconds " + (this.get() < 0 ? "before" : "after")
      : "unset";
  }
 ...
 *  @memberof Settings */
function setCanvasColorTiles(colorTiles) {
  canvasColorTiles = colorTiles;
}
...
function setGLEnable(enable) {
  ...
  glEnable = enable;
  if (glCanvas)
    // hide glCanvas if WebGL is disabled
 ...
 *  @memberof Settings */
function setObjectDefaultRestitution(restitution) {
  objectDefaultRestitution = restitution;
}
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setTouchGamepadCenterButton(enable) {
  touchGamepadCenterButton = enable;
}
...
class EngineObject {
  ...
  constructor(
    ...
    ASSERT(
      !tileInfo || tileInfo instanceof TileInfo,
      "object tileInfo should be a TileInfo or undefined"
    );
    ASSERT(
      typeof angle === "number" && isFinite(angle),
      "object angle should be a number"
    );
    ASSERT(isColor(color), "object color should be a valid rgba color");
    ASSERT(
      typeof renderOrder === "number",
      "object renderOrder should be a number"
    );
  ...
  updatePhysics() {
    ...
    if (this.groundObject) {
      ...
      const groundSpeed = this.groundObject.velocity
        ...
        : 0;
      this.velocity.x =
        groundSpeed + (this.velocity.x - groundSpeed) * friction;
      this.groundObject = undefined;
    }
    ...
    if (this.collideSolidObjects) {
      ...
      for (const o of engineObjectsCollide) {
        ...
        if (!smallStepUp && isBlockedX) {
          ...
          else this.velocity.x *= -restitution;
        }
        debugPhysics &&
    ...
    if (this.collideTiles) {
      ...
      if (hitLayer) {
        ...
        if (!tileCollisionTest(oldPos, this.size, this)) {
          ...
          const blockedLayerY = tileCollisionTest(
            vec2(oldPos.x, this.pos.y),
            this.size,
            this
          );
          const blockedLayerX = tileCollisionTest(
   ...
   *  @return {Vector2} */
  getUp(scale = 1) {
    return vec2().setAngle(this.angle, scale);
  }
   ...
   *  @return {Vector2} */
  getRight(scale = 1) {
    return vec2().setAngle(this.angle + PI / 2, scale);
  }
  ...
  renderDebugInfo() {
    ...
    if (this.parent)
      drawRect(this.pos, size.scale(0.8), rgb(1, 1, 1, 0.5), this.angle);
    this.parent && drawLine(this.pos, this.parent.pos, 0.1, rgb(1, 1, 1, 0.5));
  }
}
/**
...
function tile(
  ...
  if (typeof pos === "number") {
    ...
    const posX = pos % cols,
      posY = (pos / cols) | 0;
    tileInfo.pos.set(
      posX * sizePaddedX + padding,
      posY * sizePaddedY + padding
    );
  } else
    tileInfo.pos.set(
      pos.x * sizePaddedX + padding,
      pos.y * sizePaddedY + padding
    );
  return tileInfo;
}
...
class TileInfo {
  ...
  frame(frame) {
    ...
    return this.offset(
      new Vector2(frame * (this.size.x + this.padding * 2), 0)
    );
  }
...
function drawTile(
  ...
  ASSERT(isNumber(angle), "angle must be a number");
  ASSERT(
    !additiveColor || isColor(additiveColor),
    "additiveColor must be a color"
  );
    ...
    if (textureInfo) {
      ...
      if (bleedScale) {
        ...
        glDraw(
          ...
          size.y,
          angle,
          x + tileImageFixBleedX,
          y + tileImageFixBleedY,
          x - tileImageFixBleedX + w,
          y - tileImageFixBleedY + h,
          color.rgbaInt(),
...
function drawRectGradient(
  ...
  if (useWebGL && glEnable) {
    ...
    drawCanvas2D(
      ...
      (context) => {
        // if no tile info, use untextured rect
        const gradient = context.createLinearGradient(0, -0.5, 0, 0.5);
...
function drawLine(
  ...
  if (screenSpace) halfDelta.y *= -1; // flip angle Y if screen space
...
function drawEllipse(
  ...
  ASSERT(isNumber(lineWidth), "lineWidth must be a number");
  ASSERT(
    lineWidth >= 0 && lineWidth < size.x && lineWidth < size.y,
    "invalid lineWidth"
  );
  ASSERT(!context || !useWebGL, "context only supported in canvas 2D mode");
...
function screenToWorld(screenPos) {
  let x = (screenPos.x - mainCanvasSize.x / 2 + 0.5) / cameraScale;
  let y = (screenPos.y - mainCanvasSize.y / 2 + 0.5) / -cameraScale;
  if (cameraAngle) {
    // apply camera rotation
    const c = cos(-cameraAngle),
      s = sin(-cameraAngle);
...
function worldToScreen(worldPos) {
  ...
  return new Vector2(
    x * cameraScale + mainCanvasSize.x / 2 - 0.5,
    y * -cameraScale + mainCanvasSize.y / 2 - 0.5
  );
}
...
function inputClearKey(
  ...
  if (!inputData[device]) return;
  inputData[device][key] &= ~(
    (clearDown ? 1 : 0) |
    (clearPressed ? 2 : 0) |
    (clearReleased ? 4 : 0)
  );
  ...
  for (let i = gamepads.length; i--; ) {
    ...
    if (gamepad) {
      ...
      for (let j = gamepad.buttons.length; j--; ) {
        ...
        if (!button.value || button.value > 0.9)
          if (!i && button.pressed)
            // must be a full press
            isUsingGamepad = true;
      }
...
function touchGamepadButtonCenter() {
  ...
  if (touchGamepadButtonCount <= 2) center.x += touchGamepadSize / 2;
  return center;
}
...
function touchInputInit() {
  ...
  function handleTouch(e) {
    ...
    if (touchGamepadEnable) handleTouchGamepad(e);
  ...
  function handleTouchGamepad(e) {
    ...
    for (const touch of e.touches) {
      ...
      if (stickCenter.distance(touchPos) < touchGamepadSize) {
        ...
        if (touchGamepadButtonCount === 1) button = 0;
        ...
        if (button < touchGamepadButtonCount) touchGamepadButtons[button] = 1;
      } else if (
...
function touchGamepadRender() {
  ...
  const buttonCenter = touchGamepadButtonCenter();
  const buttonSize =
    touchGamepadButtonCount > 1 ? touchGamepadSize / 4 : touchGamepadSize / 2;
  for (let i = 0; i < touchGamepadButtonCount; i++) {
    const j = mod(i - 1, 4);
    let button =
      touchGamepadButtonCount > 2 ? j : min(j, touchGamepadButtonCount - 1);
    // fix button locations (swap 2 and 3 to match gamepad layout)
    button = button === 3 ? 2 : button === 2 ? 3 : button;
...
class Sound {
  ...
  constructor(zzfxSound, range = soundDefaultRange, taper = soundDefaultTaper) {
    ...
    if (zzfxSound) {
      ...
      const randomnessIndex = 1,
        defaultRandomness = 0.05;
      this.randomness =
        zzfxSound[randomnessIndex] !== undefined
          ? zzfxSound[randomnessIndex]
          : defaultRandomness;
   ...
   */
  getDuration() {
    return this.sampleChannels && this.sampleRate
      ? this.sampleChannels[0].length / this.sampleRate
      : 0;
  }
...
class SoundWave extends Sound {
  ...
  async loadSound(filename) {
    ...
    if (!response.ok)
      throw new Error(
        `Failed to load sound from ${filename}: ${response.status} ${response.statusText}`
      );
...
class SoundInstance {
  ...
  constructor(
    ...
    ASSERT(
      ...
      "SoundInstance requires a valid Sound object"
    );
   ...
   */
  getDuration() {
    return this.rate ? this.sound.getDuration() / this.rate : 0;
  }
...
function playSamples(
  ...
  pan = 0,
  loop = false,
  sampleRate = audioDefaultSampleRate,
  gainNode,
  offset = 0,
  onended
) {
  ...
  if (onended) source.addEventListener("ended", () => onended(source));
...
function tileCollisionGetData(pos) {
  ...
  for (const layer of tileCollisionLayers)
    if (pos.arrayCheck(layer.size)) return layer.getCollisionData(pos);
  return 0;
}
...
function tileCollisionTest(pos, size = vec2(), object, solidOnly = true) {
  for (const layer of tileCollisionLayers) {
    if (!solidOnly || layer.isSolid)
      if (layer.collisionTest(pos, size, object)) return layer;
  }
}
    ...
    for (let x = levelSize.x; x--; )
      for (let y = levelSize.y; y--; ) {
        ...
        if (data) {
          ...
          if (layerIndex === collisionLayer) tileLayer.setCollisionData(pos, 1);
        }
      }
    if (draw) tileLayer.redraw();
  }
  return tileLayers;
}
...
class CanvasLayer extends EngineObject {
  ...
  constructor(
    ...
    this.canvas = headlessMode
      ? undefined
      : new OffscreenCanvas(canvasSize.x, canvasSize.y);
  ...
  useWebGL(enable = true, immediate = false) {
    ...
    if (enable) this.textureInfo.createWebGLTexture();
    else this.textureInfo.destroyWebGLTexture();
  }
}
...
class TileLayer extends CanvasLayer {
  ...
  render() {
    ...
    const pos = this.pos.add(size.scale(0.5));
    const useWebGL = glEnable && this.textureInfo.hasWebGL();
  ...
  redrawEnd() {
    ...
    [drawCanvas, drawContext, mainCanvasSize, cameraPos, cameraScale] =
      this.savedRenderSettings;
  }
...
class TileCollisionLayer extends TileLayer {
  ...
  collisionTest(pos, size = new Vector2(), object) {
    ...
    ASSERT(
      !object || object instanceof EngineObject,
      "object must be an EngineObject"
    );
  ...
  collisionRaycast(posStart, posEnd, object, normal) {
    ASSERT(
      isVector2(posStart) && isVector2(posEnd),
      "positions must be Vector2s"
    );
    ASSERT(
      !object || object instanceof EngineObject,
      "object must be an EngineObject"
    ...
    if (hitPos) {
      const tilePos = hitPos.floor().add(vec2(0.5));
      debugRaycast && debugRect(tilePos, vec2(1), "#f008");
      debugRaycast && debugLine(posStart, hitPos, "#f00", 0.02);
      debugRaycast && debugPoint(hitPos, "#0f0");
      debugRaycast &&
        normal &&
        debugLine(hitPos, hitPos.add(normal), "#ff0", 0.02);
      return hitPos;
    }
  }
}
...
class ParticleEmitter extends EngineObject {
  ...
  constructor(
    ...
    this.emitSize = emitSize instanceof Vector2 ? emitSize.copy() : emitSize;
  ...
  update() {
    ...
    if (debugParticles) {
      // show emitter bounds
      const emitSize =
        typeof this.emitSize === "number" ? vec2(this.emitSize) : this.emitSize;
...
class Particle extends EngineObject {
  ...
  render() {
    ...
    } else drawTile(pos, size, this.tileInfo, this.color, angle, this.mirror);
...
class Medal {
  ...
  render(hidePercent = 0) {
    ...
    const backgroundColor = hsl(0, 0, 0.9);
  ...
  renderIcon(pos, size) {
    ...
    else drawTextScreen(this.icon, pos, size * 0.7, BLACK);
  }
...
const gl_MAX_POLY_VERTEXES =
  (gl_ARRAY_BUFFER_SIZE / gl_POLY_VERTEX_BYTE_STRIDE) | 0;
...
function glInit() {
  ...
  glCanvas.addEventListener("webglcontextlost", (e) => {
    ...
    for (const info of glTextureInfos) info.glTexture = undefined;
...
function glCreateTexture(image) {
  ...
  if (mipMap) glContext.generateMipmap(glContext.TEXTURE_2D);
...
function glRegisterTextureInfo(textureInfo) {
  ...
  else textureInfo.glTexture = glCreateTexture(textureInfo.image);
}
 ...
 *  @memberof WebGL */
function glDrawOutlineTransform(
  points,
  rgba,
  lineWidth,
  x,
...
function glDrawPoints(points, rgba) {
  if (!glEnable || points.length < 3) return; // needs at least 3 points to have area

  // flush if there is not enough room or if different blend mode
  const vertCount = points.length + 2;
  if (
    glBatchCount + vertCount >= gl_MAX_POLY_VERTEXES ||
    glBatchAdditive !== glAdditive
  )
    glFlush();
  glSetPolyMode();
...
function glMakeOutline(points, width, wrap = true) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const len1 = (dx1 * dx1 + dy1 * dy1) ** 0.5;

    // direction from current to next
    const dx2 = next.x - curr.x;
    const dy2 = next.y - curr.y;
    const len2 = (dx2 * dx2 + dy2 * dy2) ** 0.5;

    if (len1 < e && len2 < e) continue; // skip degenerate point
...
class PostProcessPlugin {
  ...
  constructor(shaderCode, includeOverlay = false, includeMainCanvas = true) {
    ...
    if (!shaderCode)
      ...
      shaderCode =
        "void mainImage(out vec4 c,vec2 p){c=texture(iChannel0,p/iResolution.xy);}";
    ...
    engineAddPlugin(
      undefined,
      postProcessRender,
      postProcessContextLost,
      postProcessContextRestored
    );
    ...
    function postProcessRender() {
      ...
      const uniformLocation = (name) =>
        glContext.getUniformLocation(postProcess.shader, name);
      glContext.uniform1i(uniformLocation("iChannel0"), 0);
      glContext.uniform1f(uniformLocation("iTime"), time);
      glContext.uniform3f(
        uniformLocation("iResolution"),
        mainCanvas.width,
        mainCanvas.height,
        1
      );
...
class UISystemPlugin {
  ...
  constructor(context = overlayContext) {
    ...
    this.defaultButtonColor = hsl(0, 0, 0.7);
    /** @property {Color} - Default hover color for UI elements */
    this.defaultHoverColor = hsl(0, 0, 0.9);
    /** @property {Color} - Default color for disabled UI elements */
    this.defaultDisabledColor = hsl(0, 0, 0.3);
    ...
    function uiUpdate() {
      function updateInvisibleObject(o) {
        ...
        for (const c of o.children) updateInvisibleObject(c);
        o.updateInvisible();
      }
      function updateObject(o) {
        if (o.visible) {
          // set position in parent space
          if (o.parent) o.pos = o.localPos.add(o.parent.pos);
          // update in reverse order to detect mouse enter/leave
          for (let i = o.children.length; i--; ) updateObject(o.children[i]);
          o.update();
        } else updateInvisibleObject(o);
      }
    ...
    function uiRender() {
      ...
      function renderObject(o) {
        ...
        if (o.parent) o.pos = o.localPos.add(o.parent.pos);
        o.render();
        for (const c of o.children) renderObject(c);
      }
  ...
  drawRect(
    ...
    if (gradientColor) {
      ...
      g.addColorStop(0, c);
      g.addColorStop(0.5, gradientColor.toString());
      g.addColorStop(1, c);
      context.fillStyle = g;
    } else context.fillStyle = color.toString();
    ...
    else context.rect(pos.x - size.x / 2, pos.y - size.y / 2, size.x, size.y);
    context.fill();
  ...
  drawLine(
    posA,
    posB,
    lineWidth = uiSystem.defaultLineWidth,
    lineColor = uiSystem.defaultLineColor
  ) {
    ASSERT(isVector2(posA), "posA must be a vec2");
    ASSERT(isVector2(posB), "posB must be a vec2");
    ASSERT(isNumber(lineWidth), "lineWidth must be a number");
  ...
  drawText(
    ...
    if (textShadow)
      drawTextScreen(
        ...
        font,
        fontStyle,
        applyMaxWidth ? size.x : undefined,
        0,
        uiSystem.uiContext
      );
    drawTextScreen(
   ...
   */
  screenToNative(pos) {
    if (!uiSystem.nativeHeight) return pos;
   ...
   *  @memberof Engine */
  destroyObjects() {
    for (const o of this.uiObjects) o.parent || o.destroy();
...
class UIObject {
  ...
  constructor(pos = vec2(), size = vec2()) {
    ...
    /** @property {Color} - Uses a gradient fill combined with color */
    this.gradientColor = uiSystem.defaultGradientColor
      ? uiSystem.defaultGradientColor.copy()
      : undefined;
  ...
  update() {
    ...
    if (this.isHoverObject()) {
      if (!this.disabled) {
        if (mousePress) {
          if (this.interactive) {
            if (!this.dragActivate || !wasHover || mouseWasPressed(0))
              this.onPress();
            if (this.soundPress) this.soundPress.play();
        ...
        if (!mouseDown && this.isActiveObject() && this.interactive) {
          this.onClick();
          if (this.soundClick) this.soundClick.play();
        }
      }
    ...
    if (isActive)
      if (!mouseDown || (this.dragActivate && !this.isHoverObject())) {
        this.onRelease();
        if (this.soundRelease) this.soundRelease.play();
  ...
  updateInvisible() {
    ...
    if (this.isActiveObject()) uiSystem.activeObject = undefined;
  }
  ...
  isHoverObject() {
    return uiSystem.hoverObject === this;
  }

  /** @return {boolean} - Is the mouse held onto this element */
  isActiveObject() {
    return uiSystem.activeObject === this;
  }
...
class UIText extends UIObject {
  ...
  constructor(
    ...
    ASSERT(
      ...
      "ui text align must be left, center, or right"
    );
...
class UICheckbox extends UIObject {
  ...
  constructor(
    ...
    checked = false,
    text = "",
    color = uiSystem.defaultButtonColor
  ) {
    super(pos, size);
  ...
  render() {
    ...
    if (this.checked) {
      ...
      const length = lerp(1, 2 ** 0.5 / 2, p) / 2;
      let s = this.size.scale(length);
      uiSystem.drawLine(
        this.pos.add(s.multiply(vec2(-1))),
        this.pos.add(s.multiply(vec2(1))),
        this.lineWidth,
        this.lineColor
      );
      uiSystem.drawLine(
        this.pos.add(s.multiply(vec2(-1, 1))),
        this.pos.add(s.multiply(vec2(1, -1))),
        this.lineWidth,
        this.lineColor
      );
...
class UIScrollbar extends UIObject {
  ...
  update() {
    ...
    if (this.isActiveObject() && this.interactive) {
      ...
      this.value = isHorizontal ? percent(p.x, p1, p2) : percent(p.y, p2, p1);
...
class Box2dObject extends EngineObject {
  ...
  constructor(
    ...
    angle = 0,
    color,
    bodyType = box2d.bodyTypeDynamic,
    renderOrder = 0
  ) {
  ...
  render() {
    ...
    if (this.tileInfo) super.render();
    else this.drawFixtures(this.color, this.lineColor, this.lineWidth);
  }
   ...
   *  @param {number} [lineWidth]
   *  @param {CanvasRenderingContext2D} [context] */
  drawFixtures(color = WHITE, lineColor = BLACK, lineWidth = 0.1, context) {
  ...
  addRegularPoly(
    ...
    ASSERT(
      isNumber(diameter) && diameter > 0,
      "diameter must be a positive number"
    );
    ASSERT(
      isNumber(sides) && sides > 2,
      "sides must be a positive number greater than 2"
    );

    const points = [];
    const radius = diameter / 2;
    for (let i = sides; i--; )
      points.push(vec2(radius, 0).rotate(((i + 0.5) / sides) * PI * 2));
  ...
  addRandomPoly(diameter = 1, density, friction, restitution, isSensor) {
    ...
    for (let i = sides; i--; )
      points.push(
        vec2(rand(radius / 2, radius * 1.5), 0).rotate((i / sides) * PI * 2)
      );
    return this.addPoly(points, density, friction, restitution, isSensor);
  }
   ...
   *  @return {Vector2} */
  getCenterOfMass() {
    return box2d.vec2From(this.body.GetWorldCenter());
  }

  /** Gets the linear velocity
   *  @return {Vector2} */
  getLinearVelocity() {
    return box2d.vec2From(this.body.GetLinearVelocity());
  }
  ...
  setCenterOfMass(center) {
    this.setMassData(center);
  }

  /** Set the mass of the body
   *  @param {number} mass */
  setMass(mass) {
    this.setMassData(undefined, mass);
  }
   ...
   *  @return {boolean} */
  hasFixtures() {
    return !box2d.isNull(this.body.GetFixtureList());
  }
   ...
   *  @return {boolean} */
  hasJoints() {
    return !box2d.isNull(this.body.GetJointList());
  }
...
class Box2dJoint {
  ...
  /** Destroy this joint */
  destroy() {
    box2d.world.DestroyJoint(this.box2dJoint);
    this.box2dJoint = 0;
  }
   ...
   *  @return {Box2dObject} */
  getObjectA() {
    return this.box2dJoint.GetBodyA().object;
  }

  /** Get the second object attached to this joint
   *  @return {Box2dObject} */
  getObjectB() {
    return this.box2dJoint.GetBodyB().object;
  }

  /** Get the first anchor for this joint in world coordinates
   *  @return {Vector2} */
  getAnchorA() {
    return box2d.vec2From(this.box2dJoint.GetAnchorA());
  }

  /** Get the second anchor for this joint in world coordinates
   *  @return {Vector2} */
  getAnchorB() {
    return box2d.vec2From(this.box2dJoint.GetAnchorB());
  }

  /** Get the reaction force on bodyB at the joint anchor given a time step
   *  @param {number} time
   *  @return {Vector2} */
  getReactionForce(time) {
    return box2d.vec2From(this.box2dJoint.GetReactionForce(1 / time));
  }

  /** Get the reaction torque on bodyB in N*m given a time step
   *  @param {number} time
   *  @return {number} */
  getReactionTorque(time) {
    return this.box2dJoint.GetReactionTorque(1 / time);
  }

  /** Check if the connected bodies should collide
   *  @return {boolean} */
  getCollideConnected() {
    return this.box2dJoint.getCollideConnected();
  }

  /** Check if either connected body is active
   *  @return {boolean} */
  isActive() {
    return this.box2dJoint.IsActive();
  }
}
...
class Box2dTargetJoint extends Box2dJoint {
   ...
   *  @param {Vector2} pos */
  setTarget(pos) {
    this.box2dJoint.SetTarget(box2d.vec2dTo(pos));
  }

  /** Get the target point in world coordinates
   *  @return {Vector2} */
  getTarget() {
    return box2d.vec2From(this.box2dJoint.GetTarget());
  }
   ...
   *  @return {number} */
  getMaxForce() {
    return this.box2dJoint.GetMaxForce();
  }

  /** Sets the joint frequency in Hertz
   *  @param {number} hz */
  setFrequency(hz) {
    this.box2dJoint.SetFrequency(hz);
  }

  /** Gets the joint frequency in Hertz
   *  @return {number} */
  getFrequency() {
    return this.box2dJoint.GetFrequency();
  }
}
...
class Box2dDistanceJoint extends Box2dJoint {
   ...
   *  @return {Vector2} */
  getLocalAnchorA() {
    return box2d.vec2From(this.box2dJoint.GetLocalAnchorA());
  }

  /** Get the local anchor point relative to objectB's origin
   *  @return {Vector2} */
  getLocalAnchorB() {
    return box2d.vec2From(this.box2dJoint.GetLocalAnchorB());
  }

  /** Set the length of the joint
   *  @param {number} length */
  setLength(length) {
    this.box2dJoint.SetLength(length);
  }
  ...
  setDampingRatio(ratio) {
    this.box2dJoint.SetDampingRatio(ratio);
  }

  /** Get the damping ratio
   *  @return {number} */
  getDampingRatio() {
    return this.box2dJoint.GetDampingRatio();
  }
}
...
class Box2dRopeJoint extends Box2dJoint {
   ...
   *  @param {number} length */
  setMaxLength(length) {
    this.box2dJoint.SetMaxLength(length);
  }

  /** Get the max length of the joint
   *  @return {number} */
  getMaxLength() {
    return this.box2dJoint.GetMaxLength();
  }
}
...
class Box2dRevoluteJoint extends Box2dJoint {
  ...
  constructor(objectA, objectB, anchor, collide = false) {
    ...
    jointDef.set_referenceAngle(
      objectA.body.GetAngle() - objectB.body.GetAngle()
    );
    jointDef.set_collideConnected(collide);
    super(jointDef);
  }
   ...
   *  @return {number} */
  getReferenceAngle() {
    return this.box2dJoint.GetReferenceAngle();
  }

  /** Get the current joint angle
   *  @return {number} */
  getJointAngle() {
    return this.box2dJoint.GetJointAngle();
  }

  /** Get the current joint angle speed in radians per second
   *  @return {number} */
  getJointSpeed() {
    return this.box2dJoint.GetJointSpeed();
  }

  /** Is the joint limit enabled?
   *  @return {boolean} */
  isLimitEnabled() {
    return this.box2dJoint.IsLimitEnabled();
  }

  /** Enable/disable the joint limit
   *  @param {boolean} [enable] */
  enableLimit(enable = true) {
    return this.box2dJoint.enableLimit(enable);
  }

  /** Get the lower joint limit
   *  @return {number} */
  getLowerLimit() {
    return this.box2dJoint.GetLowerLimit();
  }

  /** Get the upper joint limit
   *  @return {number} */
  getUpperLimit() {
    return this.box2dJoint.GetUpperLimit();
  }
   ...
   *  @param {boolean} [enable] */
  enableMotor(enable = true) {
    return this.box2dJoint.EnableMotor(enable);
  }

  /** Set the motor speed
   *  @param {number} speed */
  setMotorSpeed(speed) {
    return this.box2dJoint.SetMotorSpeed(speed);
  }

  /** Get the motor speed
   *  @return {number} */
  getMotorSpeed() {
    return this.box2dJoint.GetMotorSpeed();
  }

  /** Set the motor torque
   *  @param {number} torque */
  setMaxMotorTorque(torque) {
    return this.box2dJoint.SetMaxMotorTorque(torque);
  }

  /** Get the max motor torque
   *  @return {number} */
  getMaxMotorTorque() {
    return this.box2dJoint.GetMaxMotorTorque();
  }
   ...
   *  @return {number} */
  getMotorTorque(time) {
    return this.box2dJoint.GetMotorTorque(1 / time);
  }
}
...
class Box2dGearJoint extends Box2dJoint {
   ...
   *  @param {number} ratio */
  setRatio(ratio) {
    return this.box2dJoint.SetRatio(ratio);
  }

  /** Get the gear ratio
   *  @return {number} */
  getRatio() {
    return this.box2dJoint.GetRatio();
  }
}
...
class Box2dPrismaticJoint extends Box2dJoint {
   ...
   *  @return {Vector2} */
  getLocalAxisA() {
    return box2d.vec2From(this.box2dJoint.GetLocalAxisA());
  }
   ...
   *  @return {number} */
  getJointTranslation() {
    return this.box2dJoint.GetJointTranslation();
  }
  ...
  setMaxMotorForce(force) {
    return this.box2dJoint.SetMaxMotorForce(force);
  }

  /** Get the maximum motor force
   *  @return {number} */
  getMaxMotorForce() {
    return this.box2dJoint.GetMaxMotorForce();
  }
   ...
   *  @return {number} */
  getMotorForce(time) {
    return this.box2dJoint.GetMotorForce(1 / time);
  }
}
...
class Box2dWheelJoint extends Box2dJoint {
  ...
  setSpringFrequencyHz(hz) {
    return this.box2dJoint.SetSpringFrequencyHz(hz);
  }

  /** Get the spring frequency in Hertz
   *  @return {number} */
  getSpringFrequencyHz() {
    return this.box2dJoint.GetSpringFrequencyHz();
  }

  /** Set the spring damping ratio
   *  @param {number} ratio */
  setSpringDampingRatio(ratio) {
    return this.box2dJoint.SetSpringDampingRatio(ratio);
  }

  /** Get the spring damping ratio
   *  @return {number} */
  getSpringDampingRatio() {
    return this.box2dJoint.GetSpringDampingRatio();
  }
}
...
class Box2dWeldJoint extends Box2dJoint {
   ...
   *  @param {number} hz */
  setFrequency(hz) {
    return this.box2dJoint.SetFrequency(hz);
  }
...
class Box2dFrictionJoint extends Box2dJoint {
   ...
   *  @param {number} torque */
  setMaxTorque(torque) {
    this.box2dJoint.SetMaxTorque(torque);
  }

  /** Get the maximum friction torque
   *  @return {number} */
  getMaxTorque() {
    return this.box2dJoint.GetMaxTorque();
  }
}
...
class Box2dPulleyJoint extends Box2dJoint {
   ...
   *  @return {Vector2} */
  getGroundAnchorA() {
    return box2d.vec2From(this.box2dJoint.GetGroundAnchorA());
  }

  /** Get the second ground anchor
   *  @return {Vector2} */
  getGroundAnchorB() {
    return box2d.vec2From(this.box2dJoint.GetGroundAnchorB());
  }

  /** Get the current length of the segment attached to objectA
   *  @return {number} */
  getLengthA() {
    return this.box2dJoint.GetLengthA();
  }

  /** Get the current length of the segment attached to objectB
   *  @return {number} */
  getLengthB() {
    return this.box2dJoint.GetLengthB();
  }
   ...
   *  @return {number} */
  getCurrentLengthA() {
    return this.box2dJoint.GetCurrentLengthA();
  }

  /** Get the current length of the segment attached to objectB
   *  @return {number} */
  getCurrentLengthB() {
    return this.box2dJoint.GetCurrentLengthB();
  }
}
...
class Box2dMotorJoint extends Box2dJoint {
  ...
  constructor(objectA, objectB) {
    const linearOffset = objectA.worldToLocal(
      box2d.vec2From(objectB.body.GetPosition())
    );
  ...
  setLinearOffset(offset) {
    this.box2dJoint.SetLinearOffset(box2d.vec2dTo(offset));
  }

  /** Get the target linear offset, in frame A, in meters.
   *  @return {Vector2} */
  getLinearOffset() {
    return box2d.vec2From(this.box2dJoint.GetLinearOffset());
  }

  /** Set the target angular offset
   *  @param {number} offset */
  setAngularOffset(offset) {
    this.box2dJoint.SetAngularOffset(offset);
  }

  /** Get the target angular offset
   *  @return {number} */
  getAngularOffset() {
    return this.box2dJoint.GetAngularOffset();
  }
  ...
  setCorrectionFactor(factor) {
    this.box2dJoint.SetCorrectionFactor(factor);
  }

  /** Get the position correction factor in the range [0,1]
   *  @return {number} */
  getCorrectionFactor() {
    return this.box2dJoint.GetCorrectionFactor();
  }
}
...
class Box2dPlugin {
  ...
  constructor(instance) {
    ...
    listener.BeginContact = function (contactPtr) {
      const contact = box2d.instance.wrapPointer(
        contactPtr,
        box2d.instance.b2Contact
      );
  ...
  raycastAll(start, end) {
    ...
    raycastCallback.ReportFixture = function (
      fixturePointer,
      point,
      normal,
      fraction
    ) {
      const fixture = box2d.instance.wrapPointer(
        fixturePointer,
        box2d.instance.b2Fixture
      );
    ...
    debugRaycast &&
      debugLine(start, end, raycastResults.length ? "#f00" : "#00f", 0.02);
  ...
  raycast(start, end) {
    ...
    if (!raycastResults.length) return undefined;
  ...
  boxCastAll(pos, size) {
    ...
    queryCallback.ReportFixture = function (fixturePointer) {
      ...
      if (!queryObjects.includes(o)) queryObjects.push(o); // add if not already in list
      return true; // continue getting results
    };

    const aabb = new box2d.instance.b2AABB();
    aabb.set_lowerBound(box2d.vec2dTo(pos.subtract(size.scale(0.5))));
    aabb.set_upperBound(box2d.vec2dTo(pos.add(size.scale(0.5))));

    let queryObjects = [];
    box2d.world.QueryAABB(queryCallback, aabb);
    debugRaycast &&
      debugRect(pos, size, queryObjects.length ? "#f00" : "#00f", 0.02);
    return queryObjects;
  }
  ...
  boxCast(pos, size) {
    ...
    let queryObject;
    box2d.world.QueryAABB(queryCallback, aabb);
    debugRaycast && debugRect(pos, size, queryObject ? "#f00" : "#00f", 0.02);
    return queryObject;
  }
  ...
  pointCast(pos, dynamicOnly = true) {
    ...
    queryCallback.ReportFixture = function (fixturePointer) {
      ...
      if (
        dynamicOnly &&
        fixture.GetBody().GetType() !== box2d.instance.b2_dynamicBody
      )
        return true; // continue getting results
      if (!fixture.TestPoint(box2d.vec2dTo(pos))) return true; // continue getting results
      queryObject = fixture.GetBody().object;
      return false; // stop getting results
    };
    ...
    let queryObject;
    box2d.world.QueryAABB(queryCallback, aabb);
    debugRaycast && debugRect(pos, vec2(), queryObject ? "#f00" : "#00f", 0.02);
    return queryObject;
  }
   ...
   *  @param {Object} o */
  isNull(o) {
    return !box2d.instance.getPointer(o);
  }
...
async function box2dInit() {
  ...
  function box2dRender() {
    if (box2dDebug || debugPhysics) box2d.world.DrawDebugData();
  }

  // box2d debug drawing
  function setupDebugDraw() {
    // setup debug draw
    const debugLineWidth = 0.1;
    ...
    const getDebugColor = (color) => box2dColorPointer(color).scale(1, 0.8);
    ...
    debugDraw.DrawTransform = function (transform) {
      transform = box2d.instance.wrapPointer(
        transform,
        box2d.instance.b2Transform
      );
...
function drawNineSlice(
  ...
  const cornerOffset = size.scale(0.5).subtract(cornerSize.scale(0.5));
  ...
  for (let i = 4; i--; ) {
    ...
    const sidePos = cornerOffset.multiply(
      vec2(horizontal ? (i === 1 ? 1 : -1) : 0, horizontal ? 0 : i ? -1 : 1)
    );
    const sideSize = vec2(
      horizontal ? borderSize : centerSize.x,
      horizontal ? centerSize.y : borderSize
    );
    const sideTile = centerTile.offset(
      startTile.size.multiply(
        vec2(
          i === 1 ? 1 : i === 3 ? -1 : 0,
          i === 0 ? -flip : i === 2 ? flip : 0
        )
    ...
    drawTile(
      pos.add(sidePos.rotate(rotateAngle)),
      sideSize,
      sideTile,
      color,
      angle,
  ...
  for (let i = 4; i--; ) {
    ...
    const flipX = i > 1;
    const flipY = i && i < 3;
    const cornerPos = cornerOffset.multiply(
      vec2(flipX ? -1 : 1, flipY ? -1 : 1)
    );
    const cornerTile = centerTile.offset(
      startTile.size.multiply(vec2(flipX ? -1 : 1, flipY ? flip : -flip))
    );
    drawTile(
      pos.add(cornerPos.rotate(rotateAngle)),
      cornerSize,
      cornerTile,
      color,
      angle,
...
function drawThreeSlice(
  ...
  for (let i = 4; i--; ) {
    ...
    const sidePos = cornerOffset.multiply(
      vec2(
        horizontal ? (i === 1 ? 1 : -1) : 0,
        horizontal ? 0 : i ? -flip : flip
      )
    );
  ...
  for (let i = 4; i--; ) {
    ...
    const flipY = i > 1;
    const cornerPos = cornerOffset.multiply(
      vec2(flipX ? -1 : 1, flipY ? -flip : flip)
    );
    drawTile(
      pos.add(cornerPos.rotate(rotateAngle)),
      cornerSize,
